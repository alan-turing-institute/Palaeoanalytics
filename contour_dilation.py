...
# Dilate the binary image to fill in small gaps between lines
kernel = np.ones((5, 5), np.uint8)
dilated_image = cv2.dilate(binary_image, kernel, iterations=1)

# Find connected components in the dilated image
num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(dilated_image, connectivity=8, ltype=cv2.CV_32S)
