...
import os
import yaml
from pylithics.src.read_and_process import read_image, detect_lithic, find_lithic_contours, detect_scale, find_scale_contours
...
def test_detect_scale():
    filename = os.path.join('tests', 'test_images', 'RDK2_scale_3cm_600dpi.png')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('tests', 'edge_detection_scale.png'))

    print(binary_edge_sobel.shape)

def test_find_scale_contours():
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    plt.savefig(os.path.join('tests', 'contour_detection_scale.png'))

    print('Number of contours:', len(contours))
    print('Number of pixels:', max(contours[:, 1]) - min(contours[:, 1]))
